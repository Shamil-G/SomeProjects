...
procedure clear_questions(iid_registration in number, ilang in varchar2)
is
begin

-- Удалим вопросы на других языках
  delete from TEST_OPERATOR.questions_for_testing qt
  where qt.id_registration=iid_registration and qt.language!=ilang;
  commit;
end;
...
v_language      test_operator.registration.language%type;
...
begin
  ...
  select r.beg_time_testing, r.language
  into v_beg_time, v_language
  ...
  then
    ...
    clear_questions(iid_registration, v_language);
...
v_status          TEST_OPERATOR.registration.status%type;
...
--  select * from test_operator.registration r;
  ...
  omess:='';
  begin
    select id_registration,  id_person, fio, iin, id_region, lang, status
    into  v_id_registration, v_id_person, oname, v_passwd, v_id_region, olang, v_status
    from (
...
--/*
  ...
  if v_id_pc>0 and v_status!='Идёт тестирование'
  then
    update test_operator.registration r
