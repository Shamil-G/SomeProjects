CREATE OR REPLACE PACKAGE TEST_OPERATOR.UTILS is
...
CREATE OR REPLACE PACKAGE BODY TEST_OPERATOR.UTILS AS
  ...
  loop
      ...
      loop
          ...
          loop
              for cGRTB in 2..5
              loop
		insert into assignment (id_assignment, id_group_organization, id_post, id_position)
			values(num, cOrg.id_group_organization, cRuk.id_post, cEl.id_position);
                num:=num+1;
              end loop;
          end loop;
      end loop;
