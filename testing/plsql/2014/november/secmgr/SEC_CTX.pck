create or replace package secmgr.sec_ctx AS
...
create or replace package body secmgr.sec_ctx AS
...
BEGIN
  ...
  DBMS_SESSION.set_context(NAMESPACE => 'SEC_CTX', ATTRIBUTE => 'roles', VALUE => v_jobrole);
...
BEGIN
  ...
  DBMS_SESSION.CLEAR_CONTEXT(namespace => 'SEC_CTX', ATTRIBUTE => 'roles');
...
is
begin
    if instr(lower(sys_context('sec_ctx', 'roles')),'secmgr',1)>0
    then
    return 1;
    end if;
    return 0;
...
is
begin
    secmgr.sec_ctx.log('Admin role='||sys_context('sec_ctx', 'roles'));
    if instr(lower(sys_context('sec_ctx', 'roles')),'admin',1)>0
    then
    return 1;
    end if;
    return 0;
...
begin
    secmgr.sec_ctx.log('Admin role='||sys_context('sec_ctx', 'roles'));
    if instr(lower(sys_context('sec_ctx', 'roles')),'assignadmin',1)>0
    then
    return 1;
    end if;
    return 0;
...
is
begin
    if instr(lower(sys_context('sec_ctx', 'roles')),'themeadmin',1)>0
    then
    return 1;
    end if;
    return 0;
...
is
begin
--    secmgr.sec_ctx.log('role='||sys_context('sec_ctx', 'roles'));
    if instr(lower(sys_context('sec_ctx', 'roles')),'operator',1)>0
    then
    return 1;
    end if;
    return 0;
...
begin
  select (
        ...
        r.status )
  into ResI
  from test_operator.registration r, TEST_OPERATOR.persons p
