...
--    procedure picture_del(iid_question number);
    ...
    procedure questions_new(iid_question in number,
              ...
              iorder_num in number, iquestion in varchar2, idescr in varchar2);
    procedure questions_upd( iid_question number,
              iid_theme in number, ilang in char, iversion_number number,
              iorder_num in number, iactiv in char, iquestion in varchar2, idescr in varchar2 );
    ...
    procedure replies_true_new(iid_reply number, iid_question number, ireply varchar2);
    procedure replies_true_upd(iid_reply number, ireply varchar2 );
    ...
    procedure member_assignment_question_del(iid_question in number, iid_assignment in number);
    procedure member_assignment_question_new(iid_question in number, iid_assignment in number);
    procedure member_assignment_question_upd(iid_question in number, iid_assignment in number);
...
--    end picture_upd;
    ...
    procedure questions_new(iid_question in number,
              ...
              iorder_num in number, iquestion in varchar2, idescr varchar2)
    as
    ...
    insert into test_operator.questions (id_question, id_theme,
                    version_number, language, order_num_question, active, question, descr)
            values(iid_question, iid_theme,
                    iversion_number, ilang, v_order_number, 'Y', iquestion, idescr);
    commit;
    end questions_new;

    procedure questions_upd(iid_question in number,
              iid_theme in number, ilang in char, iversion_number in number,
              iorder_num in number,  iactiv in char, iquestion in varchar2, idescr varchar2)
    as
    v_count simple_integer:=0;
    begin
    ...
    update test_operator.replies r
            set r.order_num_answer=iorder_num,
                r.active=iactive,
    ...
    begin
      ...
      where r.id_question=iid_question
            and r.correctly='Y';
      raise_application_error(-20000, 'v_number: '||v_number||', iid_reply: '||iid_reply);
      if v_number=0 then
        insert into test_operator.replies (id_reply, id_question,
                        active, correctly, order_num_answer, reply)
        values(iid_reply, iid_question, 'Y', 'Y', 5, ireply);
        commit;
      else
        replies_true_upd( iid_reply, ireply);
      end if;
    end replies_true_new;
...
--    values(iid_theme, sysdate, 'Y', v_order_number, idescr, idescr_kaz);
    ...
    v_id_theme number(9);
    begin
        insert into m_assignment_questions (id_question, id_assignment) values(iid_question, iid_assignment);
    commit;
    exception when dup_val_on_index then null;
