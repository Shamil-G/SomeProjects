...
function checkUserForTesting(iid_person in number, iid_bundle in number, idate_testing in date)
return nvarchar2;
...
begin
  str:=checkUserForTesting(iid_person, iid_bundle, idate_testing );
  if (str is not null)
  then
      secmgr.sec_ctx.log(iDebug=>5,iappname=>'Registr',
            ioperation=>'Проверка кандидата на допуск к тестированию',
  ...
  values( iid_registration, iid_person, iid_bundle,
          sysdate,
      ...
      for Cur in ( select * from bundle_theme b
                 where b.id_bundle=iid_bundle
          ...
          users_bundle_composition( id_registration,
           ...
           values( iid_registration,
             ...
             if v_first_theme=true then
                v_period_testing:=case when Cur.Is_Groups='Y'
